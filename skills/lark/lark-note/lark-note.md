...
身份：`+detail` 支持 `--as user` / `--as bot`；`+transcript` 仅支持 `--as user`。`note_id` 若由某个身份取得（例如 `vc +detail --as bot`），`+detail` 必须显式沿用同一个 `--as`——不要依赖 profile 默认身份。完整身份延续规则见 [`../lark-shared/lark-shared.md`](../lark-shared/lark-shared.md)，使用前必读。
...
- `note_doc_token`：纪要正文文档，正文读取交给 [lark-doc](../lark-doc/lark-doc.md)。
- `verbatim_doc_token`：普通纪要逐字稿文档；unified 逐字稿不按这个 token 路由。

## 不在本 Skill 范围

- 通过 `meeting_id` 定位纪要（`note_id`）→ [lark-vc](../lark-vc/lark-vc.md)（`vc +detail`）。
- 通过 `minute_token` 定位纪要（`note_id`）→ [lark-minutes](../lark-minutes/lark-minutes.md)（`minutes +detail` 顶层返回 `note_id`）。
- 通过日程 `event_id` 定位会议(`meeting_id`) / 用户绑定纪要(`meeting_note`) → [lark-calendar](../lark-calendar/lark-calendar.md)（`calendar +meeting`）。
- 自然语言纪要标题搜索 → [lark-drive](../lark-drive/lark-drive.md) / [lark-doc](../lark-doc/lark-doc.md)。
- Docx 正文读取 → [lark-doc](../lark-doc/lark-doc.md)。
- 妙记基础信息与媒体文件 → [lark-minutes](../lark-minutes/lark-minutes.md)。
