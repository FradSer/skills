...
|`<sheet>`、`<cite file-type="sheets">`|提取 `token` 和 `sheet-id`，转到 [`lark-sheets`](../lark-sheets/lark-sheets.md)|
|`<bitable>`、`<cite file-type="bitable">`|提取 `token` 和 `table-id`，转到 [`lark-base`](../lark-base/lark-base.md)|
|`<vc-transcribe-tab>`|提取 `vc-node-id`，使用 [`lark-note`](../lark-note/lark-note.md) 的 `note +detail`|
