...
1. [`../lark-shared/lark-shared.md`](../lark-shared/lark-shared.md) — 认证、权限处理、全局参数（所有操作通用）
...
## 快速决策
- 用户要**复制文档 / 创建文档副本 / 另存为副本**时，切到 [`lark-drive`](../lark-drive/lark-drive.md)，按其中的复制指引使用 `lark-cli drive files copy`；不要用 `docs +fetch` + `docs +create` 重建正文，也不要走 `drive +export` / `drive +import`。
- 先判定任务路径：找文档 / 导入导出走 [`lark-drive`](../lark-drive/lark-drive.md)；只读 / 摘要用 `docs +fetch` 默认 `simple`；明确旧文本 → 新文本直接 `str_replace`；只有 block 链接、评论锚点、插入 / 替换 / 删除 / 移动才局部 fetch `with-ids`；保真改写已有内容才读 `full`
...
| 标签 / 属性 | 提取字段 | 切到技能 |
|-|-|-|
| `<sheet token="..." sheet-id="...">` | `token` -> spreadsheet_token, `sheet-id` | [`lark-sheets`](../lark-sheets/lark-sheets.md) |
| `<bitable token="..." table-id="...">` | `token` -> app_token, `table-id` | [`lark-base`](../lark-base/lark-base.md) |
| `<cite type="doc" file-type="sheets" token="..." sheet-id="...">` | 同 `<sheet>` | [`lark-sheets`](../lark-sheets/lark-sheets.md) |
| `<cite type="doc" file-type="bitable" token="..." table-id="...">` | 同 `<bitable>` | [`lark-base`](../lark-base/lark-base.md) |
| `<vc-transcribe-tab vc-node-id="...">` | `vc-node-id` -> note_id | [`lark-note`](../lark-note/lark-note.md)：先 `note +detail --note-id <vc-node-id>` |
...
- 文档评论管理 → [`lark-drive`](../lark-drive/lark-drive.md)
- 电子表格或 Base 的数据操作 → [`lark-sheets`](../lark-sheets/lark-sheets.md) / [`lark-base`](../lark-base/lark-base.md)
- 云空间文件上传、下载、权限管理 → [`lark-drive`](../lark-drive/lark-drive.md)
